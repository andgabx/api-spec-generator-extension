...
export function isJWT(token) {
  ...
  // Strict check: the header and payload must be valid JSON after base64 decode
  try {
...
/**
 * Extracts all JWTs from headers (including Authorization, Cookie, Custom headers).
 * Returns an array of objects: { token, sourceLabel }
 */
export function extractJWTsFromHeaders(headers, direction = 'Request') {
  const found = [];
  for (const h of headers) {
    ...
    if (match && isJWT(match[1])) {
      found.push({ token: match[1], sourceLabel: `${direction} Header (${h.name})` });
      continue;
    }

    // Split by common delimiters (like ';' in cookies, or ',' in some headers)
    const parts = val.split(/[;,]/);
    for (const p of parts) {
      const eqIdx = p.indexOf('=');
      const candidate = eqIdx > -1 ? p.slice(eqIdx + 1).trim() : p.trim();
      
      // Some tokens might still have "Bearer " prefix inside a custom header
      const cleanCandidate = candidate.replace(/^Bearer\s+/i, '');
      if (isJWT(cleanCandidate)) {
        found.push({ token: cleanCandidate, sourceLabel: `${direction} Header (${h.name})` });
      }
    }
  }
  return found;
...
/**
 * Extracts JWTs from URL query parameters
 */
export function extractJWTsFromUrl(urlString) {
  const found = [];
  try {
    const url = new URL(urlString);
    url.searchParams.forEach((val, key) => {
      if (isJWT(val)) {
        found.push({ token: val, sourceLabel: `URL Parameter (?${key})` });
      }
    });
  } catch {
    // Ignore invalid URLs
